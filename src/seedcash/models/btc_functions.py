...
def derivation_m_44_145_0(hexa_seed):
    ...
    private_master_key, private_master_code = get_private_and_code(hexa_seed)

    # Derivem amb index 44'   m/ a m/44' de forma endurida i optenim una child_key i un child_chain_code,
    purpose_index = 44
    purpose_key, purpose_chain_code = child_key_hardened(
    ...
    coin_type_index = 145
    coin_type_key, coin_type_chain_code = child_key_hardened(
    ...
    account_index = 0
    account_key, account_chain_code = child_key_hardened(
        coin_type_key, coin_type_chain_code, account_index, hardened=True
    )
    account_public_key = public_master_key_compressed_generaitor(account_key)
    ...
    # finerprint del pare
    father_acount_publickey = public_master_key_compressed_generaitor(coin_type_key)
    father_fingerprint = fingerprint_bytes(father_acount_publickey)
...
def fingerprint_hex(hexa_seed):
    ...
    (
        ...
        account_key,
        account_public_key,
    ) = derivation_m_44_145_0(hexa_seed)
...
def xpub_to_legacy_address(xpub, address_index):
    ...
    version, depth, fingerprint, child_number, chain_code_chain, public_key_chain = (
        xpub_decode(xpub)
    )  # m/44'/145'/0'

    child_public_chain, child_chain_chain = derive_public_child_key(
        public_key_chain, chain_code_chain, 0
    )  # m/44'/145'/0'/0
    child_public_address_index, child_chain_address_index = derive_public_child_key(
        child_public_chain, child_chain_chain, address_index
    )  # m/44'/0'/0'/0/0
    address = public_key_to_legacy_address(child_public_address_index)
...
def create_checksum(prefix, payload):
    ...
    polymod_result = polymod(values + [0, 0, 0, 0, 0, 0, 0, 0])
    return [(polymod_result >> (5 * (7 - i))) & 0x1F for i in range(8)]


def public_key_to_cashaddr_address(pubkey):
    version_byte = 0x00  # Per P2PKH
    payload = bytes([version_byte]) + hash160(pubkey)
    payload_5bit = convert_bits(payload, 8, 5)
    checksum = create_checksum("bitcoincash", payload_5bit)
    address = "bitcoincash:" + encode_base32(payload_5bit + checksum)
    return address
...
def xpub_to_cashaddr_address(xpub, address_index):
    ...
    address = public_key_to_cashaddr_address(child_public_address_index)
    return address
...
def xpriv_2_direction_wif(xpriv, chain, address_index):
    ...
    chain_child_key_bytes, chain_chain_code_bytes = ckd_privatekey(
        private_key, chain_code, chain
    )

    address_child_key_bytes, b = ckd_privatekey(
    ...
    wif = private_key_to_wif_bch(address_child_key_bytes, compressed=True)
    return wif
