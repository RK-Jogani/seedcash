    ...
    def _parse_outputs(self):
        ...
        for i, out in enumerate(self.psbt.outputs):
            ...
            if self.psbt.tx.vout[i].script_pubkey.data[0] == OPCODES.OP_RETURN:
                ...
                # For BCH, prefer CashAddr for legacy p2pkh/p2sh outputs
                spk = self.psbt.tx.vout[i].script_pubkey
                try:
                    st = spk.script_type()
                except Exception:
                    st = None
                ...
                if st in ["p2pkh"]:
                    # p2pkh script format: OP_DUP OP_HASH160 0x14 <20 bytes> OP_EQUALVERIFY OP_CHECKSIG
                    data = spk.data
                    hash160 = data[3:23]
                    addr = Bip44.hash160_to_cashaddr(hash160)
                elif st in ["p2sh"]:
                    # p2sh script format: OP_HASH160 0x14 <20 bytes> OP_EQUAL
                    data = spk.data
                    hash160 = data[2:22]
                    addr = Bip44.hash160_to_cashaddr(hash160)
                else:
